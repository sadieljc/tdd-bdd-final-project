...
class TestProductRoutes(TestCase):
    ...
    def test_query_by_name(self):
        ...
        data = response.get_json()
        self.assertEqual(len(data), have_same_name)
    ...
    def test_query_by_category(self):
        ...
        response = self.client.get(BASE_URL, query_string=f"category={quote_plus(str(category.name))}")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_query_by_availability(self):
        ...
        logging.debug("Total available products: %s", count_available_products)

        response = self.client.get(BASE_URL + "?available=true")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        data = response.get_json()
        self.assertEqual(len(data), count_available_products)
